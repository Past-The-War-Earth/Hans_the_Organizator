...
module to.edit {
  ...
  export class AddAfterActionsToActionController extends common.ActionsController {
    ...
    (
      ...
      super($ionicHistory, $scope, $state, categoryRepository, motivationRepository, 'AddAfterActionsToActionController', 'Add Do After');
      this.reload();
    }
    ...
    reload():Promise<any> {
      return this.actionRepository.get(this.$stateParams.actionId).then(( action )=> {
        this.action = action;
        this.actionService.getCandidatesForAfterAction(action, this.categoryFilter, this.motivationFilter, this.adviceFilter).then(( candidateActions ) => {
    ...
    private filter():void {
      this.actionService.getCandidatesForAfterAction(this.action, this.categoryFilter, this.motivationFilter, this.adviceFilter).then(( candidateActions ) => {
        this.beforeActions = candidateActions;
        this.$scope.$apply();
      });
    }
    ...
    selectAction(
      ...
      this.actionService.addAfterAction(this.action, action).then(() => {
        this.back();
      });
    }
